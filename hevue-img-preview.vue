<template>
    <transition name="fade">
        <div class="hevue-imgpreview-wrap" id="hevue-imgpreview-wrap" v-if="visible" ref="heImg" @touchend="clickMaskFn">
            <div class="he-img-wrap" @touchstart="addMoveMobile">
                <div class="heimgfont hevue-img-status-icon rotate-animation" v-show="imgState === 1">&#xe6b1;</div>
                ...
                <!-- transform: scale(${imgScale}) rotate(${imgRotate}deg) translate3d(${imgLeft}px, ${imgTop}px, 0px); ${maxWH} -->
                <!-- transform: scale(${imgScale}) rotate(${imgRotate}deg) translateZ(0px); margin-top: ${imgTop}px; margin-left: ${imgLeft}px; ${maxWH} -->
                <!-- 图片加载失败 -->
                <div class="heimgfont hevue-img-status-icon" v-show="imgState === 3">&#xec0d;</div>
                <!-- 关闭按钮 -->
                <div class="heimgfont he-close-icon" @touchend.stop="close({ way: 'closeBtn' })" v-if="closeBtn">&#xe608;</div>
                <!-- 左箭头 -->
                <div class="arrow arrow-left heimgfont" @touchend.stop="toogleImg(false, 'btn')" v-if="arrowBtn && multiple">&#xe620;</div>
                <!-- 右箭头 -->
                <div class="arrow arrow-right heimgfont" @touchend.stop="toogleImg(true, 'btn')" v-if="arrowBtn && multiple">&#xe60d;</div>
                <!-- 控制条 -->
                <div class="he-control-bar-wrap" v-if="controlBar">
                    <div class="he-control-bar" @touchend.stop>
                        <!-- 缩小 -->
                        <div class="he-control-btn heimgfont" @touchend="scaleFunc(-0.15)">&#xe65e;</div>
                        <!-- 放大 -->
                        <div class="he-control-btn heimgfont" @touchend="scaleFunc(0.15)">&#xe65d;</div>
                        <!-- 复位 -->
                        <div class="he-control-btn heimgfont" @touchend="imgToggle">
                            <span v-if="isFull">&#xe698;</span>
                            <span v-else>&#xe86b;</span>
                        </div>
                        <!-- 左转 -->
                        <div class="he-control-btn heimgfont" @touchend="rotateFunc(-90)">&#xe670;</div>
                        <!-- 右转 -->
                        <div class="he-control-btn heimgfont" @touchend="rotateFunc(90)">&#xe66f;</div>
                        <!-- 下载 -->
                        <!-- <div class="he-control-btn heimgfont" @click="downloadIamge">&#xe694;</div> -->
                    </div>
                </div>
                <!-- 页码指示器 -->
                <div class="he-control-num" v-if="controlBar && multiple">{{ imgIndex + 1 }} / {{ imgList.length }}</div>
            </div>
        </div>
    </transition>
</template>
...
export default {
    ...
    methods: {
        ...
        changeUrl(url, index) {
            // let img = new Image();
            let img = this.$refs.heImView;
            img.onload = () => {
                console.log("onload", img.sizes, img.height, img.width);
                clearTimeout(this.loadImgTimeId)
            ...
            this.imgurl = url;
            this.loadImgTimeId = setTimeout(() => this.imgState = 1, 200)
        },
        // 旋转图片
        rotateFunc(deg) {
            this.imgRotate += deg;
        ...
        addMoveMobile(e) {
            ...
            console.log("touchstart", e.touches, this.isClickEvent);
        },
        // 鼠标拖动
        moveFunc(e) {
            e = e || window.event;
        ...
        moveFuncMobile(e) {
            ...
            console.log("touchmove", e.touches, this.isClickEvent);
        },
        // 移除拖动事件
        removeMove(type) {
            if (type === "pc") {
        ...
        // 点击遮罩层
        clickMaskFn(e) {
            console.log("clickMaskFn", e.touches, this.isClickEvent);
            clearTimeout(this.clickDelay);
